  ...
  def stock_cost(inventory_id = nil)
    ...
    Rails.cache.fetch([:stock_cost, id, inventory_id.to_i ]) do
      stock_cost = if inventory_product_info.need_serial
        ...
        grn_batches.to_a.sum{|g| g.inventory_batch.inventory_id == inventory_id ? (g.grn_item.current_unit_cost.to_f * (g.remaining_quantity.to_f + g.damage_quantity.to_f)) : 0 }
        ...
        inventories.where(id: inventory_id).each do |inventory|
          ...
          total_sum += grn_items.only_grn_items1.to_a.sum{|g| g.grn.store_id.to_i == store_id.to_i ? (g.remaining_quantity.to_f + g.damage_quantity.to_f) * g.current_unit_cost.to_f : 0 } if store_id.present?
...
class InventoryProductInfo < ActiveRecord::Base
  ...
  validates_presence_of :product_id, if: Proc.new {|i| i.product_id.present? }
...
class InventoryCategory1 < ActiveRecord::Base
  ...
  has_many :active_inventory_category2s, -> {where(active: true)}, foreign_key: :category1_id, class_name: "InventoryCategory2"
...
class InventoryCategory2 < ActiveRecord::Base
  ...
  has_many :active_inventory_category3s, -> {where(active: true)}, foreign_key: :category2_id, class_name: "InventoryCategory3"
  has_many :inventory_category3s, foreign_key: :category2_id
...
class InventoryPo < ActiveRecord::Base
  ...
  before_save do |po|
    if po.persisted? and po.remarks_changed? and po.remarks.present?
      po_remarks = "#{po.remarks} <span class='pop_note_e_time'> on #{Time.now.strftime('%d/ %m/%Y at %H:%M:%S')}</span> by <span class='pop_note_created_by'> #{User.cached_find_by_id(po.current_user_id).try(:full_name)}</span><br/>#{po.remarks_was}"
    elsif po.new_record?
...
class InventoryPrn < ActiveRecord::Base
  ...
  before_save do |prn|
    if prn.persisted? and prn.remarks_changed? and prn.remarks.present?
      prn_remarks = "#{prn.remarks} <span class='pop_note_e_time'> on #{Time.now.strftime('%d/ %m/%Y at %H:%M:%S')}</span> by <span class='pop_note_created_by'> #{User.cached_find_by_id(prn.current_user_id).try(:full_name)}</span><br/>#{prn.remarks_was}"
