class Ticket < ActiveRecord::Base
    ...
    tire.search(all_page) do
      query do
        boolean do
          ...
          must { range :logged_at, lte: params[:l_range_to].to_date.end_of_day } if params[:l_range_to].present?
          must { range :logged_at, gte: params[:l_range_from].to_date.beginning_of_day } if params[:l_range_from].present?
  ...
  def to_indexed_json
    ...
    to_json(
      ...
      include: {
        ...
        customer: {
          only: [:id, :organization_id, :district_id],
          methods: [:full_name, :full_address, :district_name],
          include: {
            contact_type_values: {
        ...
        ticket_payment_receiveds: {
          only: [:id, :amount, :received_at, :currency_id],
          methods: [:received_by_user_full_name]
        },
        customer_quotations: {
  ...
  def ticket_product_brand_name
    products.first.try(:product_brand).try(:name)
  end
  def ticket_product_brand_id
    products.first.try(:product_brand).try(:id)
  end
  ...
  def update_on_create_ticket_info
    ...
    company_config.increase_after_assign_ticket_no self
  ...
  def re_open_ticket(re_open_action_id, engineer_id = nil)
    ...
    until current_engineer.blank?
      new_engineer = ticket_engineers.build current_engineer.attributes.select{|a| ["user_id", "sbu_id", "re_assignment_requested", "channel_no", "task_description"].include? a}
    ...
    to_json(
      ...
      include: {
        ticket_contract: {
          ...
          methods: [:num_of_products, :brand_name, :brand_id, :category_cat_id, :category_cat1_id, :category_cat2_id, :category_name, :category_full_name_index, :payment_type, :formated_created_at, :created_by_user_full_name, :formated_contract_start_at, :formated_contract_end_at, :dynamic_active, :formated_accepted_at, :product_amount, :discount_less_amount, :contract_no_genarate],
          include: {
            organization: {
              only: [:id, :name, :code, :updated_at],
              methods: [:get_organization_account_manager],
