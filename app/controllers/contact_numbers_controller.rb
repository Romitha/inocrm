class ContactNumbersController < ApplicationController
    ...
    respond_to do |format|
      if @contact_number.save
        format.html {redirect_to (@contact_number.c_numberable_type == "User" ? profile_user_path(current_user) : polymorphic_path([:edit, @contact_number.c_numberable])), notice: "contact_number is successfully created"}
        ...
        format.html {redirect_to (@contact_number.c_numberable_type == "User" ? profile_user_path(current_user) : polymorphic_path([:edit, @contact_number.c_numberable]))}
  ...
  def destroy
    ...
    # respond_with(@contact_number)
    respond_to do |format|
      format.html {redirect_to polymorphic_url([@contact_number.c_numberable]), notice: "contact_number is successfully deleted."}
    end
  end

  def make_primary_contact_number
    contact_number.where(primary: true).each do |contact_number|
  ...
  private
    def set_contact_number
      @contact_number = contact_number.find(params[:id])
    end
