class TicketsController < ApplicationController
  ...
  def customer_summary
    ...
    render json: {id: @customer.id, email: @customer.email, full_name: @customer.full_name, phone_number: @customer.primary_contact_number.try(:value), address: @customer.primary_address.try(:address), nic: @customer.NIC, avatar: view_context.image_tag((@customer.avatar.thumb.url || "no_image.jpg"), alt: @customer.email)}
  end
      ...
      begin
        Ticket.transaction do
          ...
          if @ticket.save!
            ...
            @ticket.ticket_product_serials.where( product_serial_id: @product.id ).first.update(location_address_id: @product.location_address_id) if @ticket.ticket_product_serials.where( product_serial_id: @product.id ).first.present?
  ...
  def load_sbu
    ...
    when "sbu"
      @sbus = if params[:filter_sbu].present?
        ...
        @sbus = sbu.sbu_engineers.joins(:engineer).where(active: true, users: {active: true}).map { |s| {id: s.engineer.id, active: s.engineer.active, name: "#{s.engineer.full_name} (#{ TicketEngineer.where(user_id: s.engineer_id, job_completed_at: nil).count })"} }
  ...
  def customer_advance_payment
    ...
    if params[:customer_name].present?
      search_customer_name = params[:customer_name]
      @tickets = Kaminari.paginate_array(Ticket.joins(:customer).where(" spt_ticket.status_id != ? and spt_ticket.cus_payment_required = ? and spt_ticket.cus_payment_completed = ?", TicketStatus.find_by_code("CLS").id, true, false ).where("spt_customer.name like ?", "%#{search_customer_name}%")).page(params[:page]).per(100)
    else
      @tickets = Kaminari.paginate_array(Ticket.where("spt_ticket.status_id != ? and spt_ticket.cus_payment_required = ? and spt_ticket.cus_payment_completed = ?", TicketStatus.find_by_code("CLS").id, true, false )).page(params[:page]).per(100)
    end
...
#        Set Action:(66) Reject Close Ticket, and  DB.spt_act_ticket_close_approve
  ...
  def create_po
    ...
    po_no = po_params['po_no']
    @po = if SoPo.find_by_po_no(po_no).present?
      SoPo.find_by_po_no(po_no)
    else
      SoPo.new
    end
